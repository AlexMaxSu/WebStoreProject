	...
	<div style="padding-top: 30px;">
		<table style="width: 700px;border-style:1px solid " border="1">
			...
			<c:forEach items="${shoppingCar.shoppingItem}" var="item">
				<tr id="tr${item.value.product.pid}">
					<td><input type="checkbox" class="ids" name="ids" value="${item.value.product.pid}" checked></td>
					<td><img src="${item.value.product.imgurl}" width="100" height="100" /></td>
					<td>${item.value.product.pname}</td>
					<td>${item.value.subcount}</td>
					<td>${item.value.product.estoreprice}</td>
					<td id="price">${item.value.subtotal}</td>
				</tr>
				<c:set var="sum" value="${shoppingCar.total}"> </c:set>
			</c:forEach>
		</table>
	</div>
